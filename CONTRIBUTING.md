...
**Path A: keep your own repository.** Write it however you like, push it, then
generate a registration and open a pull request with it:

```bash
keel module register \
  --repo <you>/<your-repo> --commit <full-sha> \
  --entry lib/whatever.mjs \
  --id your-module --category render --owner-user your-handle \
  --out modules/your-handle/render/your-module
```

That fetches your repository at that commit, rebuilds it, and writes
`keel.registration.json` recording what the rebuild produced: source digest,
output digest, receipt digest, archive digest, and every file the entry
actually imports. If it does not reproduce, nothing is written. You cannot
register a claim about your own code that a rebuild did not produce.

Your source is never copied here. The registration is the only file that lands
in this repository.
...
A registration commits the digests rather than looking them up, and that split
is deliberate.

`keel module index` reads only committed files, never the network. If indexing
fetched your repository, the catalog would depend on what GitHub served that
afternoon, and re-running it could quietly produce a different catalog. Instead
indexing is offline and deterministic: regenerate it on a clean checkout and
you get a byte-identical file, which is what lets anyone check the published
catalog by rebuilding it and running `diff`.

Re-deriving those digests is a separate, networked verb:

```bash
keel module verify --all
```

It re-fetches every registered origin, rebuilds it, and compares against what
the registration committed. Drift is a hard failure and names what moved,
whether that is the forge serving a different tree for the same commit or a
